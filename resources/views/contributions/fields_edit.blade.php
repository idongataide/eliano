...
<div class="form-group">
    ...
    <div class="col-sm-8">
            <select class="form-control" id="contributor_id" name="contributor_id" required>
                ...
                @foreach ($user as $row)
                    <option   {{ ($row->id == $data->contributor_id)? 'selected':'' }} value="{{ $row->id }}">{{ $row->name. ' - '.$row->reg_no }}</option>
                @endforeach
          </select>
    </div>
</div>
    ...
    <div class="col-sm-8">
            <select class="form-control" id="payment_method_id" name="payment_method_id">
                ...
                @foreach ($paymentmethod as $row)
                    <option   {{ ($row->id == $data->payment_method_id)? 'selected':'' }} value="{{ $row->id }}">{{ $row->name }}</option>
                @endforeach
          </select>
    </div>
</div>
